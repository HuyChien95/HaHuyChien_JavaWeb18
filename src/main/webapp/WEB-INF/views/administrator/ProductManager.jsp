...
<body>
	...
	<div class="layout-wrapper layout-content-navbar">
			...
			<div class="layout-page">
				...
				<div class="content-wrapper">
					...
					<div class="container-xxl flex-grow-1 container-p-y">
						<form class="form-inline" action="${base}/admin/productlist" method="get">
							<div class="d-flex flex-row">
								...
								<select class="form-control" name="categoryId" id="categoryId"
									...
									<c:forEach var="parent" items="${categoryParent}">
									<optgroup label="${parent.name}">
									<c:forEach var="Child" items="${categoryChild}">
										<c:if test="${parent.id == Child.parent.id}">
													...
													<span id="_product_status_${product.id}"> 
														<c:choose>
															<c:when test="${product.isHot}">
																<p style="color: blue; font-weight: bold;">yes</p>
															</c:when>
															<c:otherwise>
													...
													<span id="_product_status_${product.id}"> 
														<c:choose>
															<c:when test="${product.status}">
																<p style="color: blue; font-weight: bold;">show</p>
															</c:when>
															<c:otherwise>
																<c:out value="hide"></c:out>
															</c:otherwise>
														</c:choose>
													</span>
												</td>
												<td>
													<a class="dropdown-item" href="${base}/admin/editproduct/${product.id}"><i class="bx bx-edit-alt me-2"></i> Edit</a>
													<c:choose>
															<c:when test="${product.status}">
																<div class="dropdown-item" onclick="HSBtn(${product.id})"><i class="bx bx-hide me-2"></i> Hide</div>
															</c:when>
															<c:otherwise>
																<div class="dropdown-item" onclick="HSBtn(${product.id})"><i class="bx bx-show me-2"></i> Show</div>
															</c:otherwise>
													</c:choose>
												</td>
											</tr>
		...
		function HSBtn(productId){
			...
			jQuery.ajax({
				url: "${base}" + "/admin/productlist/delete", //->request mapping
